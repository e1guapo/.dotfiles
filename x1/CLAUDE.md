...
`files/etc/` is a *source tree* for system-level config that requires `sudo install` via an explicit installer script. Guix Home does not touch `/etc`. Editing a file under `files/etc/` has zero effect until its installer is re-run (`install_keyd_config.sh`, `install_logind_lid_policy.sh`). Treat these like a Makefile with manual targets.
